...
p_neut :: DDecay
p_neut = d (neut, [t gravitino, t jets, t jets])
...
map_n1n1 = 
    HM.fromList [ (Nothing       , MGProc [] [ "p p > n1 n1 " ])
                , (Just (3,1000022,[]) , MGProc [] [ "n1 > grv j j " ] ) 
		, (Just (4,1000022,[]) , MGProc [] [ "n1 > grv j j " ] )
                ] 
...
pdir :: ProcDir
pdir = ProcDir { pdWorkDirPrefix = "grv_2jet" 
               , pdRemoteDirBase = "grv_2jet"
               , pdRemoteDirPrefix = "grv_2jet" }
...
main = do 
  ...
  genPhase1 GldGrv ssetup pdir proc GldGrvParam (NumOfEv 10000,SetNum 1)
  genPhase2 GldGrv ssetup pdir proc GldGrvParam (NumOfEv 10000,SetNum 1)
